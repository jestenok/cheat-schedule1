...
public:
    ...
    struct Assembly final {
        ...
        [[nodiscard]] auto Get(const std::string& strClass, const std::string& strNamespace = "*", const std::string& strParent = "*") const -> Class* {
            if (classes.empty()) {
                printf("Assembly %s has no classes\n", name.c_str());
                return nullptr;
            };
            for (const auto pClass : classes) {
                if (strClass == pClass->name && (strNamespace == "*" || pClass->namespaze == strNamespace) &&
                    (strParent == "*" || pClass->parent == strParent))
                    return pClass;
            }
            return nullptr;
        }
...
#if WINDOWS_MODE
        if (!address_.contains(funcName) || !address_[funcName]) address_[funcName] = (void*)GetProcAddress(static_cast<HMODULE>(hmodule_), funcName.c_str());
#elif  ANDROID_MODE || LINUX_MODE || IOS_MODE || HARMONYOS_MODE
...
public:
    ...
    public:
        ...
        struct RaycastHit {
            Vector3 m_Point;
            Vector3 m_Normal;
            char filler[0x038 - sizeof(Vector3) * 2];
            int32_t m_Collider;

            auto get_collider() -> Collider* {
                static Method* method;

                if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("RaycastHit")->Get<Method>("get_collider");
                if (method) return method->Invoke<Collider*>(this);
                return {};
            }
        };
